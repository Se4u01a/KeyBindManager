...
using RawInputObserver = std::function<bool(uint32_t, KeyEventType)>;
...
public:
    ...
    void SetRawInputObserver(RawInputObserver observer);
...
private:
    ...
    void DispatchPolledKey(std::uint32_t dxScanCode, KeyEventType eventType);
    ...
    std::atomic<bool> _mousePollingStarted = false;
    std::jthread _mousePollThread;
    RawInputObserver _rawInputObserver;
